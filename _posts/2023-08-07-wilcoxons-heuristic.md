...
While machine computers did exist at that time, most calculations were still performed with pencil, paper, and slide rule. (If you had to use a slide rule in school, you have my congratulations and condolences.) And precalculated tables were prepared for ease of use in doing statistical calculations. What is particularly innovative about the heuristic of using ranks instead of the original data is that it alleviated the need to have many different tables of values for the various distributions that might be utilized. It accomplishes this by taking advantage of the mathematical fact that for many rank transforms on collections of independent random variables ([dense set](https://en.wikipedia.org/wiki/Dense_set) [images](https://en.wikipedia.org/wiki/Image_(mathematics))) that their derived distribution by change of variables are almost surely uniform. So instead of having separate tables for normal distributions, $t$-distributions, $\chi^2$ distributions, etc, you can instead of a single table of values which assumes this underlying uniformity in distribution of ranks. I'll casually refer to such variables as *independent continuous random variables* although the terminology is not without problems when it comes to what we mean by "[continuity](https://en.wikipedia.org/wiki/Continuous_function)".
...
Let us return the notion that ranking collections of independent random variables gives uniformly-distributed random variables. We can understand this with an example. For [IID](https://en.wikipedia.org/wiki/Independent_and_identically_distributed_random_variables) samples of size $m=10^4$ we can sample instances of each of the following random variables

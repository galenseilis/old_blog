...
First, let us suppose that there is a sequence of $n$ tiles labelled $1-n$. It has been known for decades that the classic game of Snakes and Ladders can be modelled as a Markov chain (Daykin *et al* 1967). Simply put, there are a collection of states in which a player's game piece can be positioned and the probability of going to any other position only depends on the current position. This is known as the Markov property.

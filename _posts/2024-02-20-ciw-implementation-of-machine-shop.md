...
We can also run multiple simulations to obtain an aggregate result, and then plot it.

![](assets/images/ciw_impl_simpy_machine_shop.png)

The above plot is generated by the following code if you want to tinker.

```python
import matplotlib.pyplot as plt

plt.style.use('Solarize_Light2')
pd.concat(tuple([CiwMachineShop().run() for i in range(1000)])).hist(bins=100)
plt.savefig('ciw_impl_simpy_machine_shop.png', dpi=300, transparent=True)
plt.close()
```

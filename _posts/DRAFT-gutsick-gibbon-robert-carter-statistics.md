...
In short, no, the average of a ratio is not the same as the ratio of the averages in general. The [expectation operator is linear](https://en.wikipedia.org/wiki/Expected_value#Properties), so it distributes over [linear combinations](https://en.wikipedia.org/wiki/Linear_combination). A ratio of two random variables is not a linear combination.
